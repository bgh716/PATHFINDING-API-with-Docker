...
fn get_length(a: types::Args) -> types::Result {
	let mut map = Vec::new();
	for i in 0..10{
		map.push(a.my_map[i].to_vec());
	}
	let mut start = Vec::new();
	start.push(a.start_loc[0].to_vec());
	let mut goal = Vec::new();
	goal.push(a.goal_loc[0].to_vec());
	
	let IF = SAPF::UI::new(map, start, goal, 1);
	let solution = IF.find_solution();
